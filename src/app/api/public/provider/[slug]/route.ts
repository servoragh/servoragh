...
export async function GET(
  ...
  try {
    ...
    if (profile) {
      const artisanName = profile.user?.name || "Eng. Rashid Mohammed";
      const expYears = 5;
      ...
      result = {
        ...
        category: profile.tagline || "Solar & Heavy Power Solutions",
        ...
        reviewsCount: profile.reviewsCount || 36,
        reviewCount: profile.reviewsCount || 36,
        ...
        phone: profile.phone || profile.whatsappNumber || profile.user?.phone || "+233240000000",
        whatsappNumber: profile.whatsappNumber || profile.phone || profile.user?.phone || "+233240000000",
        zone: coverage,
        ...
        catalogs: {
          ...
          services: profile.services.map((s) => ({
            ...
            startingPrice: s.startingPrice ? Number(s.startingPrice) : 150.0,
            estimatedTime: s.estimatedDuration || "2-4 Hours",
          })),
          rentals: profile.rentals.map((r) => ({
            ...
            depositRequired: r.securityDeposit ? Number(r.securityDeposit) : 500.0,
      ...
      if (provider) {
        const artisanName = provider.user?.name || "Eng. Rashid Mohammed";
        const expYears = provider.yearsExperience || 1;
        const coverage = provider.serviceArea || "Tamale, Bolgatanga, Wa, Yendi, All Northern Region";
        ...
        result = {
          ...
          verificationTier: provider.verificationStatus || "TIER_3_REGISTERED_ENTERPRISE",
          isVerified: true,
          logoUrl: provider.logoUrl || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=500&auto=format&fit=crop&q=80",
          ...
          rating: provider.ratingAverage || 5.0,
          reviewsCount: provider.reviewCount || 36,
          reviewCount: provider.reviewCount || 36,
          completedJobsCount: provider.completedJobsCount || 85,
          completedJobs: provider.completedJobsCount || 85,
          hourlyRate: provider.pricingHourly ? `GH₵ ${Number(provider.pricingHourly).toFixed(2)}/hr` : "GH₵ 100.00/hr",
          startingPrice: provider.pricingFixedStart ? `GH₵ ${Number(provider.pricingFixedStart).toFixed(2)}` : "GH₵ 250.00",
          responseSpeed: "< 15 minutes",
          coverageZones: [coverage],
          aboutText: provider.bio || "Northern Ghana's leading distributor of high-efficiency solar panels, lithium wall batteries, pure sine wave inverters, and heavy water pump generators.",
          ...
          bio: provider.bio || "Northern Ghana's leading distributor of high-efficiency solar panels, lithium wall batteries, pure sine wave inverters, and heavy water pump generators.",
          catalogs: {
            ...
            services: provider.services.map((ps) => ({
              ...
              description: ps.service?.description || "Service",
              startingPrice: 150.0,
        ...
        // 3. Default Tamale Solar & Heavy Power Solutions Fallback Profile
