...
export function UnifiedMessagingHub({
  ...
  useEffect(() => {
    fetchRooms();
    const interval = setInterval(fetchRooms, 4000); // Polling real-time delivery
  ...
  useEffect(() => {
    if (activeRoomId) {
      ...
      const interval = setInterval(() => fetchRoomDetails(activeRoomId), 3000);
      return () => clearInterval(interval);
    }
  }, [activeRoomId]);
  ...
  async function handleSendMessage(e: React.FormEvent) {
    e.preventDefault();
    if (!activeRoomId || (!messageInput.trim() && !attachmentUrl)) return;
    ...
    try {
      ...
      if (data.moderationFlagged) {
        setModerationWarning(
          "⚠️ Notice: Phone number, email, or off-platform payment detected and masked according to Servora Security Policy."
        );
      }
  ...
  // Filter channels by scope & search query
  ...
  const getScopeBadge = (scope: string) => {
    switch (scope) {
      case "C2B":
        return { label: "Buyer ↔ Vendor", icon: MessageSquare, color: "bg-emerald-600/20 text-emerald-400 border-emerald-500/30" };
      case "C2ADMIN_SUPPORT":
        return { label: "Helpdesk Ticket", icon: UserCheck, color: "bg-blue-600/20 text-blue-400 border-blue-500/30" };
      case "B2ADMIN":
        return { label: "Merchant Ops", icon: Building2, color: "bg-purple-600/20 text-purple-400 border-purple-500/30" };
      case "B2B_WHOLESALE":
        return { label: "Wholesale B2B", icon: Sparkles, color: "bg-amber-600/20 text-amber-400 border-amber-500/30" };
      case "DISPUTE_MEDIATION":
        return { label: "3-Way Dispute", icon: Scale, color: "bg-rose-600/20 text-rose-400 border-rose-500/30" };
      case "LOGISTICS_COURIER":
        return { label: "Courier Order", icon: Truck, color: "bg-cyan-600/20 text-cyan-400 border-cyan-500/30" };
      case "BOT_HELP":
        return { label: "AI Bot Triage", icon: Bot, color: "bg-indigo-600/20 text-indigo-400 border-indigo-500/30" };
      default:
        return { label: scope, icon: MessageSquare, color: "bg-stone-800 text-stone-300 border-stone-700" };
    }
  };

  return (
    <div className="bg-stone-900 border border-stone-800 rounded-3xl overflow-hidden shadow-2xl h-[720px] flex flex-col md:flex-row text-white text-xs">
      {/* LEFT PANEL: CHANNELS & INBOX LIST */}
      <div className="w-full md:w-80 border-r border-stone-800 flex flex-col bg-stone-950/60">
        ...
        <div className="p-4 border-b border-stone-800 space-y-3">
          ...
          {/* Search Box */}
          <div className="flex items-center gap-2 bg-stone-800/80 px-3 py-1.5 rounded-xl border border-stone-700/60">
        ...
        {/* Room List */}
        <div className="flex-1 overflow-y-auto divide-y divide-stone-850/60">
          {loadingRooms ? (
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3.5 transition flex items-start gap-3 ${
                    isActive ? "bg-stone-850 border-l-4 border-emerald-500" : "hover:bg-stone-900"
                  }`}
                >
                  <div className="w-10 h-10 rounded-xl bg-stone-800 text-white font-bold flex items-center justify-center shrink-0 border border-stone-700">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      ...
                      {room.unreadCount > 0 && (
                        <span className="w-4 h-4 bg-emerald-500 text-stone-950 font-black text-[10px] rounded-full flex items-center justify-center">
                          {room.unreadCount}
                        </span>
                      )}
                    </div>
      ...
      <div className="flex-1 flex flex-col bg-stone-900">
        {activeRoomData ? (
          <>
            ...
            <div className="p-4 bg-stone-850 border-b border-stone-800 flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <h4 className="font-bold text-sm text-white">{activeRoomData.title || "Conversation Thread"}</h4>
                  <span
              ...
              {currentUserRole === "ADMIN" && (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl font-bold text-xs flex items-center gap-1.5 transition ${
                    isInternalNote ? "bg-amber-600 text-white" : "bg-stone-800 text-amber-400 border border-amber-600/40"
                  }`}
                >
            ...
            {moderationWarning && (
              <div className="p-3 bg-amber-950/80 border-b border-amber-800 text-amber-300 text-xs flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <ShieldAlert className="w-4 h-4 text-amber-400 shrink-0" />
                  <span>{moderationWarning}</span>
                </div>
                <button onClick={() => setModerationWarning(null)} className="font-bold text-white hover:underline">
                  Dismiss
                </button>
              </div>
            )}
            ...
            {/* Message Stream */}
            <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-stone-925">
                  ...
                  >
                    <div
                      className={`max-w-[75%] p-3.5 rounded-2xl space-y-1.5 ${
                        isNote
                          ? "bg-amber-950/90 text-amber-200 border border-amber-700/80"
                          : isMe
                      ...
                      {attachments.length > 0 && (
                        <div className="pt-2">
                          <img
                            src={attachments[0]}
                            alt="Attachment"
                            className="max-h-48 rounded-xl object-cover border border-stone-700"
                          />
                        </div>
                      )}
                    </div>
            ...
            {/* Message Input Box */}
            <form onSubmit={handleSendMessage} className="p-3 bg-stone-850 border-t border-stone-800 space-y-2">
              {isInternalNote && (
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 p-3 bg-stone-900 text-white placeholder-stone-400 rounded-2xl outline-none border border-stone-700 font-medium"
                />

                {/* Upload Image Button */}
                <label className="p-3 bg-stone-800 hover:bg-stone-700 text-stone-300 rounded-2xl cursor-pointer border border-stone-700 shrink-0">
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-stone-500">
            <MessageSquare className="w-12 h-12 mb-3 text-stone-700" />
            <h4 className="text-base font-bold text-white mb-1">Select a Channel to Start Chatting</h4>
