  ...
  async function handleSubmitRequest() {
    ...
    try {
      const res = await fetch("/api/requests", {
        ...
        body: JSON.stringify({
          serviceId: isCustomCategory ? null : selectedServiceId || null,
          customCategory: isCustomCategory ? customCategory : null,
          tags: tagsArray,
          title: title || (isCustomCategory ? customCategory : "Service Request"),
          description,
          media: attachments,
    ...
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-stone-900 border border-stone-800 rounded-3xl w-full max-w-xl overflow-hidden shadow-2xl transition-all text-white text-xs">
        ...
        <div className="bg-gradient-to-r from-emerald-900 via-emerald-800 to-stone-900 p-6 flex items-center justify-between border-b border-stone-800">
          ...
          <button onClick={onClose} className="p-2 rounded-xl bg-stone-800/80 hover:bg-stone-700 text-stone-400 hover:text-white transition">
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Progress Bar */}
        <div className="w-full bg-stone-950 h-1.5">
        ...
        <div className="p-6 space-y-4">
          ...
          {verifyingGuest ? (
            <div className="py-6 space-y-4 text-center">
              <div className="w-14 h-14 bg-amber-950 text-amber-400 rounded-2xl flex items-center justify-center mx-auto border border-amber-800/80">
                <Key className="w-7 h-7" />
              </div>
              <h4 className="text-lg font-bold text-white">Verify Phone & Claim Request</h4>
              <p className="text-stone-400 text-xs max-w-sm mx-auto">
                We sent a 4-digit code to <strong className="text-amber-400">{guestPhone}</strong>. Enter it below to publish your job & claim your temporary account!
              </p>

              {simulatedOtp && (
                <div className="p-2.5 bg-emerald-950/80 border border-emerald-800 text-emerald-300 rounded-xl text-xs font-mono max-w-xs mx-auto">
              ...
              <form onSubmit={handleVerifyGuestOtp} className="max-w-xs mx-auto space-y-3">
                <input
                  ...
                  className="w-full p-3.5 bg-stone-850 border border-stone-700 rounded-xl text-center text-lg font-mono font-bold tracking-widest text-emerald-400 outline-none"
                  required
                />
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow transition"
                >
            ...
            <div className="text-center py-6 space-y-4">
              ...
              <h4 className="text-xl font-bold text-white">Service Request Broadcasted!</h4>
              ...
              <div className="flex flex-col gap-2 pt-2">
                ...
                <button onClick={onClose} className="w-full py-2.5 text-stone-400 hover:text-white">
                  Close Window
                </button>
              </div>
            </div>
            ...
            <>
              ...
              {step === 1 && (
                <div className="space-y-4">
                  ...
                  {/* Category Picker */}
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Select Service Category</label>
                        ...
                        <button
                          ...
                          className={`p-2.5 rounded-xl text-left font-bold transition border ${
                            (isCustomCategory && cat.id === "custom") || selectedServiceId === cat.id
                              ...
                              : "bg-stone-850 border-stone-800 text-stone-400 hover:bg-stone-800 hover:text-white"
                          }`}
                        >
                          {cat.name}
                        </button>
                  ...
                  {isCustomCategory && (
                    <div>
                      <label className="block text-stone-300 font-semibold mb-1">Specify Custom Service Name</label>
                      <input
                        type="text"
                        ...
                        className="w-full p-3 bg-stone-800 border border-stone-700 text-white rounded-xl outline-none font-bold"
                      />
                    </div>
                  )}
                  ...
                  {/* Title & Description */}
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Job Title</label>
                    <input
                      type="text"
                      placeholder="e.g. Broken Samsung Screen / Fuse Replacement in Sakasaka"
                      value={title}
                      onChange={(e) => setTitle(e.target.value)}
                      className="w-full p-3 bg-stone-800 border border-stone-700 text-white rounded-xl outline-none"
                      required
                    />
                  </div>

                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Detailed Problem Description</label>
                    <textarea
                      rows={3}
                  ...
                  {/* Tags */}
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Tags / Keywords (Comma separated)</label>
                    <input
                      type="text"
                      placeholder="wiring, generator, solar, instant"
                      value={tagsInput}
                      onChange={(e) => setTagsInput(e.target.value)}
                      className="w-full p-2.5 bg-stone-800 border border-stone-700 text-stone-300 rounded-xl outline-none"
                    />
                  </div>

                  {/* Multi-file Attachments */}
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Upload Diagnostic Media (Photos, Video, PDF)</label>
                    <div className="border border-dashed border-stone-700 p-4 rounded-xl text-center bg-stone-850 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-stone-400">
                        <Upload className="w-5 h-5 text-emerald-400" />
                        <span>{uploadingFile ? "Uploading file..." : "Attach photo, short video or invoice PDF"}</span>
                      </div>
                      <label className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl cursor-pointer">
                    ...
                    {attachments.length > 0 && (
                      <div className="flex items-center gap-2 mt-2 overflow-x-auto">
                        {attachments.map((att, idx) => (
                          <span key={idx} className="px-2.5 py-1 bg-stone-800 text-emerald-300 rounded-lg font-mono text-[10px] border border-stone-700 truncate max-w-[150px]">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-emerald-950/60 border border-emerald-800/80 rounded-2xl space-y-2">
                    <div className="flex items-center justify-between">
                      <div>
                        <h5 className="font-bold text-white text-xs">One-Tap Device GPS Location</h5>
                        <p className="text-[11px] text-stone-400">Fetch exact latitude & longitude for emergency dispatch.</p>
                      </div>
                      <button
                        type="button"
                        onClick={handleFetchDeviceLocation}
                        disabled={gettingGps}
                        className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl flex items-center gap-1.5 shrink-0 shadow"
                      >
                    ...
                    {latitude && (
                      <div className="text-[11px] font-mono text-emerald-300 bg-stone-900 p-2 rounded-lg border border-emerald-800">
                  ...
                  <div className="p-3 bg-stone-850 border border-stone-800 rounded-2xl flex items-center justify-between">
                    <div>
                      <span className="font-bold text-white block">Live GPS Tracking Opt-In 🛰️</span>
                      <span className="text-[10px] text-stone-400">Stream position to provider while en route for emergency response.</span>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Landmark / Neighborhood (Tamale)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Street Address (Private until quote accepted)</label>
                    <input
                      type="text"
              ...
              {step === 3 && (
                <div className="space-y-4">
                  ...
                  {/* Urgency Status */}
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Urgency Status</label>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { id: "EMERGENCY_ASAP", label: "🚨 Emergency (ASAP)", color: "border-rose-600 text-rose-300 bg-rose-950/60" },
                        { id: "SAME_DAY", label: "⚡ Same Day", color: "border-amber-600 text-amber-300 bg-amber-950/60" },
                        { id: "SCHEDULED", label: "📅 Scheduled Date", color: "border-purple-600 text-purple-300 bg-purple-950/60" },
                        { id: "FLEXIBLE", label: "🌱 Flexible / Open", color: "border-emerald-600 text-emerald-300 bg-emerald-950/60" },
                      ].map((urg) => (
                        <button
                          key={urg.id}
                          type="button"
                          onClick={() => setUrgency(urg.id as any)}
                          className={`p-2.5 rounded-xl font-bold border text-left transition ${
                            urgency === urg.id ? urg.color : "bg-stone-850 border-stone-800 text-stone-400 hover:bg-stone-800"
                          }`}
                        >
                          {urg.label}
                        </button>
                  ...
                  {urgency === "SCHEDULED" && (
                    <div>
                      <label className="block text-stone-300 font-semibold mb-1">Preferred Date & Time</label>
                      <input
                        type="datetime-local"
                  ...
                  {/* Pricing Type */}
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Pricing Structure</label>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-stone-300 font-semibold mb-1">Min Budget (GH₵)</label>
                      <input
                        type="number"
                        placeholder="e.g. 50"
                        value={budgetMin}
                        onChange={(e) => setBudgetMin(e.target.value)}
                        className="w-full p-3 bg-stone-800 border border-stone-700 text-emerald-400 font-bold rounded-xl outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-stone-300 font-semibold mb-1">Max Budget (GH₵)</label>
                      <input
                        type="number"
                        placeholder="e.g. 250"
                        value={budgetMax}
              ...
              {step === 4 && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-stone-300 font-semibold mb-1">WhatsApp Phone</label>
                      <input
                        type="text"
                        placeholder="+233500000000"
                        value={guestPhone}
                        onChange={(e) => setGuestPhone(e.target.value)}
                        className="w-full p-3 bg-stone-800 border border-stone-700 text-white rounded-xl outline-none font-mono"
                        required
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Gate / Entry Access Instructions (Optional)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-stone-300 font-semibold mb-1">Preferred Communication</label>
                    <select
              ...
              <div className="flex items-center justify-between pt-4 border-t border-stone-800">
                {step > 1 ? (
                  <button
                    ...
                    className="px-4 py-2.5 text-stone-400 hover:text-white font-bold flex items-center gap-1"
                  >
                ...
                {step < 4 ? (
                  <button
                    type="button"
                    onClick={() => setStep((s) => s + 1)}
                    className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl flex items-center gap-1.5 shadow"
                  >
                    <span>Next Step</span>
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-lg flex items-center gap-2 transition"
                  >
